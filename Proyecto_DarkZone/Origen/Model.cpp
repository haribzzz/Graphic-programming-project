...
// Cache global: clave = ruta completa del archivo, valor = ID de textura ya cargado en GPU
...
{
    // 1. Si empieza con '-', el string tiene opciones MTL; tomamos el �ltimo token
    ...
    if (mesh->mMaterialIndex >= 0)
    {
        ...
        if (material->GetTexture(aiTextureType_DIFFUSE, 0, &texPath) == AI_SUCCESS)
        {
            ...
            if (!filename.empty())
            {
                ...
                // Buscar en el cache antes de cargar de disco
                auto it = g_textureCache.find(fullPath);
                if (it != g_textureCache.end())
                {
                    tex.id = it->second; // ya estaba cargada, reutilizamos el ID
                }
                else
                {
                    std::cout << "  Diffuse: " << fullPath << std::endl;
                    tex.id = TextureFromFile(fullPath.c_str());
                    g_textureCache[fullPath] = tex.id;
                }
