...
void LoadCollisionMesh(const char* path, float scale)
{
    ...
    std::cout << "Malla de colision cargada: " << collisionMesh.size() << " triangulos." << std::endl;
}
...
bool CollidesWithMesh(glm::vec3 pos)
{
    ...
    for (const Triangle& tri : collisionMesh)
    {
        float minX = glm::min(glm::min(tri.a.x, tri.b.x), tri.c.x) - RADIUS;
        float maxX = glm::max(glm::max(tri.a.x, tri.b.x), tri.c.x) + RADIUS;
        float minZ = glm::min(glm::min(tri.a.z, tri.b.z), tri.c.z) - RADIUS;
        float maxZ = glm::max(glm::max(tri.a.z, tri.b.z), tri.c.z) + RADIUS;
        float minY = glm::min(glm::min(tri.a.y, tri.b.y), tri.c.y);
        float maxY = glm::max(glm::max(tri.a.y, tri.b.y), tri.c.y) + 12.0f;
...
const glm::vec3 doorPos = glm::vec3(-8.0f, -5.5f, -43.7f);
const glm::vec3 door2Pos = glm::vec3(-0.32f, -5.5f, -22.64f);

// ⬇️ POSICIONES PARA INTERACCIÓN (a la altura de los ojos del jugador)
const glm::vec3 doorInteractPos = glm::vec3(-8.0f, GROUND_LEVEL, -43.7f);
const glm::vec3 door2InteractPos = glm::vec3(-0.32f, GROUND_LEVEL, -22.64f);
...
glm::vec3 llavePosicion = glm::vec3(12.0f, MAZE_FLOOR_Y + 0.5f, 21.3f);
...
void processInput(GLFWwindow* window)
{
    ...
    // =====================================
    // Helper lambda: devuelve true si el jugador esta dentro de la caja AABB
    ...
    // Escalera — pos (-27, MAZE_FLOOR_Y, -2), escala 0.05
    if (hitBox(-28.5f, -25.5f, -3.5f, 0.5f))
        cameraPos = previousPos;

    // Mesa 2 + Computadora — pos (-25, MAZE_FLOOR_Y, 3), escala 1.2
    if (hitBox(-26.5f, -23.5f, 1.5f, 4.5f))
        cameraPos = previousPos;

    // Mesa habitacion 1 — pos (12, MAZE_FLOOR_Y, 18), escala 0.02
    // La mesa es grande a pesar de la escala pequeña del modelo
    if (hitBox(10.0f, 14.5f, 16.5f, 19.5f))
        cameraPos = previousPos;

    // Librera habitacion 1 — pos (9, MAZE_FLOOR_Y+4.5, 26), escala 1.0
    if (hitBox(7.5f, 10.5f, 24.0f, 28.0f))
        cameraPos = previousPos;

    // Capsula habitacion 2 — pos (-4, MAZE_FLOOR_Y, -27), escala 1.5
    if (hitBox(-6.0f, -2.0f, -29.0f, -25.0f))
        cameraPos = previousPos;

    // Cajas habitacion 2 — pos (-11, MAZE_FLOOR_Y, -17), escala 40
    if (hitBox(-13.0f, -9.0f, -19.0f, -15.0f))
        cameraPos = previousPos;

    // Especimen habitacion 3 — pos (62, MAZE_FLOOR_Y+1, -31), escala 6.5
    if (hitBox(59.0f, 65.0f, -34.0f, -28.0f))
        cameraPos = previousPos;

    // Esqueleto habitacion 3 — pos (62, MAZE_FLOOR_Y, -22), escala 0.6
    if (hitBox(61.0f, 63.0f, -23.5f, -20.5f))
        cameraPos = previousPos;

    // Mesa lab habitacion 1 — pos (17, MAZE_FLOOR_Y, 25), escala 0.4
    if (hitBox(15.0f, 19.0f, 23.0f, 27.0f))
        cameraPos = previousPos;

    // Laboratorio set pasillo — pos (54, MAZE_FLOOR_Y, -20), escala 0.4
    if (hitBox(51.5f, 56.5f, -22.5f, -17.5f))
        cameraPos = previousPos;

    // Powerbox 1 — pos (-12, MAZE_FLOOR_Y+1.5, -22), escala 3
    if (hitBox(-13.5f, -10.5f, -23.5f, -20.5f))
        cameraPos = previousPos;

    // Powerbox 2 — pos (60, MAZE_FLOOR_Y+1.5, -19.8), escala 3
    if (hitBox(58.5f, 61.5f, -21.3f, -18.3f))
        cameraPos = previousPos;

    // Lavabo habitacion 3 — pos (52, MAZE_FLOOR_Y, -30), escala 0.4
    if (hitBox(50.5f, 53.5f, -31.5f, -28.5f))
        cameraPos = previousPos;

    // Sillas pasillo derecho — 4 sillas en Z=38.7, X=5,15,25,35
    for (float sx : {5.0f, 15.0f, 25.0f, 35.0f}) {
        if (hitBox(sx - 1.2f, sx + 1.2f, 37.5f, 39.9f))
            cameraPos = previousPos;
    }

    // Sillas pasillo lateral — 3 sillas en X=42.5, Z=-28.7,-18.7,-8.7
    for (float sz : {-28.7f, -18.7f, -8.7f}) {
        if (hitBox(41.3f, 43.7f, sz - 1.2f, sz + 1.2f))
            cameraPos = previousPos;
    }
}
...
int main()
{
    ...
    LoadCollisionMesh("Textures/mapa/Laboratorio.obj", MAZE_SCALE);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // ── PUERTAS — habilitadas ──
        // ✅ USAR doorInteractPos y door2InteractPos para la detección
        ...
        // ── DIBUJAR PUERTAS ──
        {
            // Puerta 1 — Marco2 (Habitacion derecha)
            glm::mat4 m = glm::mat4(1.0f);
            m = glm::translate(m, glm::vec3(-8.0f, -5.5f, -43.7f));
        ...
        {
            // Puerta 2 — Marco3 (Habitacion izquierda)
            glm::mat4 m = glm::mat4(1.0f);
            m = glm::translate(m, glm::vec3(-0.1f, -5.5f, -0.0f));
        ...
        //sillas 1
        {
            glm::mat4 t = glm::mat4(1); t = glm::translate(t, glm::vec3(5.0f, MAZE_FLOOR_Y, 38.7f));
        ...
        if (lookingDoor) {
            if (!doorOpen && tieneLlave) {
                ...
                RenderizarTexto(hudCX, hudCY, "Locked - Key required",
                    (int)(SCREEN_W / HUD_SCALE), (int)(SCREEN_H / HUD_SCALE));
            }
        }
        ...
        if (powerbox1Activado || lampara3Activa || lampara1Activa)
            RenderizarTexto(hudX, (int)(SCREEN_H * 0.150f), "Room 1 lamp: ON and Room 2 lamp: ON",
                (int)(SCREEN_W / HUD_SCALE), (int)(SCREEN_H / HUD_SCALE));
        if (powerbox2Activado || lampara4Activa || lampara2Activa)
            RenderizarTexto(hudX, (int)(SCREEN_H * 0.60f), "Room 3 lamp: ON and Room 4 lamp: ON",
                (int)(SCREEN_W / HUD_SCALE), (int)(SCREEN_H / HUD_SCALE));
